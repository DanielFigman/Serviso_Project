...
const FavoritesScreen = () => {
  const { updatedActivities, activities_hotel, activities_nearBy } =
    useContext(HotelsAppContext);
  const [cardsToShow, setCardsToShow] = useState([]);
  const { language } = useContext(HotelsAppContext);
  const screenContent = Languages.FavoritesScreen;

  const generateCardsToShow = () => {
    let setVal = [];

    const likedIds = updatedActivities
      .filter((obj) => obj.favorite)
      .map((obj) => obj.placeID);

    let activitiesHotelToSet = activities_hotel.filter((obj) =>
      likedIds.includes(obj.placeID)
    );
    let activitiesNearBy = activities_nearBy.filter((obj) =>
      likedIds.includes(obj.placeID)
    );
    ...
    setVal = setVal
      .concat(
        activitiesHotelToSet.map((item) => ({
          ...item,
          type: "activities_hotel",
        })),
      ...
      <ScrollView>
        ...
        >
          {cardsToShow?.map((item, index) => (
            <View
              key={item.placeID + index + item.type}
              style={{ width: "50%" }}
            >
  ...
  return (
    <ScreenComponent
      title={
        ...
        >
          {screenContent.Favorites[language]}
        </Text>
      }
      content={<View>{renderItems()}</View>}
    />
  );
};
