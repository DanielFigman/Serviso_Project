...
const RoomServiceCartCard = ({ item, cart, setCart }) => {
  ...
  useEffect(() => {
    if (quantity) {
      ...
      } else if (prevQuantityRef.current && prevQuantityRef.current > quantity && quantity > 0) {
        handleDecrease();
      }
    }
  ...
  const handleDelete = () => {
    if (item.type) {
      let isSuchObjectInCart = cart?.find(obj => obj.ID === item.ID && obj.itemsCount === item.itemsCount);
      if (isSuchObjectInCart) {
        const newCart = cart.filter(obj => !(obj.ID === item.ID && obj.itemsCount === item.itemsCount));
        setCart([...newCart]);
      }
    } else {
  ...
  const handleIncrease = () => {
    if (item.type) {
      ...
      if (isSuchObjectInCart) {
        ...
        const objectIndex = cart.findIndex(obj => obj.ID === item.ID && obj.itemsCount === item.itemsCount);
        const newCart = [...cart];
  ...
  const handleDecrease = () => {
    if (item.type) {
      let isSuchObjectInCart = cart?.find(obj => obj.ID === item.ID && obj.changes === item.changes);
      if (isSuchObjectInCart) {
        isSuchObjectInCart.amount--;
        const objectIndex = cart.findIndex(obj => obj.ID === item.ID && obj.changes === item.changes);
        const newCart = [...cart];
        newCart.splice(objectIndex, 1, isSuchObjectInCart);
        setCart(newCart);
      }
    ...
    return (
      <View style={{ flexDirection: "column", flex: 1 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-around" }}>
          <Animated.Text style={[styles.iconContainer, { opacity: trans, textAlign: "center", fontFamily: "Courier New", fontWeight: "bold", fontSize: 15 }]}>
            {item.name}
          </Animated.Text>
          <Animated.Text style={[styles.iconContainer, { opacity: trans, textAlign: "center", fontFamily: "Courier New", fontWeight: "bold", fontSize: 15 }]}>
        ...
        <View style={styles.rightActionsContainer}>
          <TouchableOpacity style={styles.incrementButton} onPress={() => handleQuantityChange('PLUS')}>
            <Animated.View style={[styles.iconContainer, { opacity: trans }]}>
              <PlusIcon size={30} color="black" />
            </Animated.View>
          </TouchableOpacity>
          <TouchableOpacity style={styles.deleteButton} onPress={handleDelete}>
            <Animated.View style={[styles.deleteIconContainer, { opacity: trans }]}>
              <TrashIcon size={30} color="black" />
            </Animated.View>
          </TouchableOpacity>
          <TouchableOpacity style={styles.decrementButton} onPress={() => handleQuantityChange('MINUS')}>
            <Animated.View style={[styles.iconContainer, { opacity: trans }]}>
