...
const AddToCartDialog = ({
  ...
  const screenContent = Languages.AddToCartDialog;
  ...
  const addToCart = () => {
    let isSuchObjectInCart = cart?.find(
      (obj) => obj.ID === item.ID && obj.changes === changes
    );
    if (isSuchObjectInCart) {
      ...
      if (item.type) {
        let foodAndDrinksCount =
          cart?.filter((obj) => obj.type !== undefined)?.length ?? 0;
        foodAndDrinksCount++;

        while (
          cart?.find((obj) => obj.type && obj.itemsCount === foodAndDrinksCount)
        ) {
          foodAndDrinksCount++;
        }
  ...
  return (
    <>
      <Dialog
        ...
        overlayStyle={
          keyBoardDidShow ? [styles.overlay, { top: -120 }] : styles.overlay
        }
        animationType="slide"
      ...
      >
        <View
          ...
          {...(!keyBoardDidShow ? panResponder.panHandlers : {})}
        >
          <LoadingImage
          ...
          <View style={styles.plusMinus}>
            ...
            <Text
              style={
                isPressedWithZero
                  ? { marginHorizontal: 15, fontSize: 18, color: "red" }
