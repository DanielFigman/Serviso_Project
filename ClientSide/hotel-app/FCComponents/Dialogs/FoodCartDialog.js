...
const FoodCartDialog = ({
  ...
  const screenContent = Languages.FoodCartDialog;
  ...
  const GetRequestObject = () => {
    ...
    const foodAndDrinksRoomService = _.cloneDeep(
      getTheNeededPropsFromFoodAndDrinks()
    );
    const additionalItems = _.cloneDeep(getTheNeededPropsFromAdditional());

    // setting the grand child to his parent
    room_Service_Order["Food_And_Drinks_Room_Service"] =
  ...
  const getTheNeededPropsFromFoodAndDrinks = () => {
    const retVal = cart
      ?.filter((obj) => obj.type)
      .map(({ ID, amount, changes, itemsCount }) => ({
        ID,
        amount,
        changes,
        itemsCount,
  ...
  return (
    ...
    >
      <View style={styles.container}>
        <Text style={styles.title}>{screenContent.YourCart[language]}</Text>
        <ScrollView
        ...
        >
          ...
          >
            <Text style={styles.text}>
              {screenContent.TotalPrice[language]}
            </Text>
            <Text
              style={{ left: 10, fontSize: 10, top: 8, fontWeight: "bold" }}
            >
              ₪
            </Text>
