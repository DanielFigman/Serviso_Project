...
const FoodCartDialog = ({ totalPrice, modalVisible, setModalVisible, cart }) => {
    ...
    const renderCards = () => {
        return tempCart?.map((obj, index) => (
            <RoomServiceCartCard item={obj} key={obj.ID + "0" + index} />
        ));
    };
    ...
    const getTotal = () => {
        ...
        tempCart.forEach(obj => total += obj.amount * obj.price)
