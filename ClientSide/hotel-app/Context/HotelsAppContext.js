...
export default function HotelsAppContextProvider(props) {
  ...
  const checkIfAlreadyScheduledAcleaning = async () => {
    try {
      const response = await fetch(
        `http://proj.ruppin.ac.il/cgroup97/test2/api/getCleaningRequest?email=${user.email}`,
        {
          method: "GET",
          headers: new Headers({
  ...
  useEffect(() => {
    ...
    if (foodAndDrinksOOS && foodAndDrinksOOS.length > 0) {
      if (food) {
        const newFood = food.filter(
          (obj) => !foodAndDrinksOOS.includes(obj.ID)
        );
      ...
      if (alcohol) {
        const newAlcohol = alcohol.filter(
          (obj) => !foodAndDrinksOOS.includes(obj.ID)
        );
    ...
    if (additionalItemsOOS && additionalItemsOOS.length > 0) {
      if (additionalItems) {
        const newAdditional = additionalItems?.filter(
          (obj) => !additionalItemsOOS.includes(obj.ID)
        );
  ...
  const postQuestionnaire = async () => {
    try {
      const response = await fetch(
        "http://proj.ruppin.ac.il/cgroup97/test2/api/updateQuestionnaire",
        {
          method: "POST",
  ...
  const setNewQuestionnaire = (questionnaireObject) => {
    ...
    Object.keys(questionnaireObject).forEach((key) => {
      const val =
        questionnaireObject[key] || questionnaireObject[key] == 0
          ? questionnaireObject[key]
          : 5;
  ...
  useEffect(() => {
    if (user && user.email) {
      ...
      if (
        notificationToken &&
        notificationToken !== prevNotificationTokenRef.current
      ) {
  ...
  useEffect(() => {
    if (updatedActivities !== null && updatedActivities !== undefined) {
      updatedActivitiesRef.current = JSON.parse(
        JSON.stringify(updatedActivities)
      );

      if (updatedActivitiesInitRef.current === null) {
        updatedActivitiesInitRef.current = JSON.parse(
          JSON.stringify(updatedActivities)
        );
      }
    }
  ...
  useEffect(() => {
    ...
    const handleAppStateChange = (nextAppState) => {
      if (nextAppState === "background") {
        ...
        if (
          updatedActivitiesRef?.current?.length !==
            updatedActivitiesInitRef?.current?.length ||
          updatedActivitiesRef?.current?.some(
            (activity, index) =>
              !isEqual(activity, updatedActivitiesInitRef.current[index])
          )
        ) {
          postFavorite();
        }
  ...
  const postFavorite = () => {
    try {
      ...
      fetch(
        `http://proj.ruppin.ac.il/cgroup97/test2/api/updateFavsAndRatings?&email=${emailRef.current}`,
        {
          method: "POST",
  ...
  const getNeededFavsToPost = () => {
    ...
    updatedActivitiesRef.current.forEach((updatedActivityRef) => {
      ...
      if (
        !matchingInitActivity ||
        !isEqual(updatedActivityRef, matchingInitActivity)
      ) {
  ...
  const fetchNToken = async () => {
    try {
      const response = await fetch(
        "http://proj.ruppin.ac.il/cgroup97/test2/api/NTUPDATE",
        {
          method: "PUT",
          body: JSON.stringify({
            email: user.email,
  ...
  const getFavorite = (placeID) => {
    ...
    const fav =
      filteredActivities.length > 0 ? filteredActivities[0].favorite : null;
